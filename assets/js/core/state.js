window.createInitialState = function () {
  return {
    ...
    eurUsd: 1.1,
    usdUah: 44.7,
    // Звідки цей курс насправді взявся. `default` — захардкоджені значення
    // вище, тобто зріз ratesAsOf, а не сьогодні: за ними рахується і акциз
    // (він у ПКУ в євро), і пороги пенсійного збору (вони в гривнях), тож
    // мовчати про це не можна. `nbu` — свіжа відповідь або кеш молодший за
    // добу; `stale` — протухлий кеш, узятий бо НБУ не відповів.
    ratesSource: "default", // 'default' | 'nbu' | 'stale'
    ...
    //   vin      — як прийшов з аукціону, у IAAI завжди замаскований;
    //   vinFull  — зчитаний з фото заводської таблички, живе в БД окремо;
    //   vinPlate — посилання на те саме фото, щоб було звідки переписати;
    //   lotId    — рядок у lots, кому дописувати vinFull.
    currentLot: {
      auction: "",
      lotNumber: "",
      vin: "",
    ...
    // Поле вводу повного VIN і статус його збереження.
    vinInput: "",
    vinMsg: "",
    ...
    // Наскільки локації можна вірити. Порожньо — лота ще не розбирали.
    //   ok     — назва філії збіглася;
    //   weak   — збігся лише штат, філія підібрана (до $375 різниці);
    //   none   — штату в лоті немає взагалі, локація лишилась дефолтною
    //            (перший рядок довідника, Алабама) разом із наземним плечем
    //            І узбережжям, тобто ставкою фрахту — а число виглядає
    //            так само впевнено, як зматчене;
    //   manual — обрано руками, попередження зняте.
    locationMatch: "",
    autoShipping: {
      location: {
