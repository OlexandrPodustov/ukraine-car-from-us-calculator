function __createAllMethods(){
  return {
                    ...
                    parseAuctionLot: async function () {
                      ...
                      for (var pi = 0; pi < proxies.length && !html; pi++) {
                        try {
                          ...
                          var tid = setTimeout(function () { ctrl.abort(); }, 13000);
                          ...
                          if (resp.ok) {
                            ...
                            var isCfBlocked = /just a moment|enable js|cloudflare/i.test(txt.slice(0, 3000));
                            console.log('[proxy] ###' + pi + ' cloudflare blocked:', isCfBlocked);

                            if (txt && txt.length > 2000 && !isCfBlocked) {
                              html = txt;
                              console.log('[proxy] ###' + pi + ' ✅ accepted');
                            } else {
                              console.warn('[proxy] ###' + pi + ' ❌ rejected — length:', txt.length, '| cf blocked:', isCfBlocked);
                            }
                          } else {
                        ...
                        if (!html) {
                          vm.auctionStatus = 'error';
                          vm.auctionMsg = '❌ Не вдалося завантажити сторінку (Cloudflare/бот-захист).';
                          return;
                        }
                        ...
                        try {
                          ...
                          var nextMatch = html.match(/<script[^>]+id="ProductDetailsVM"[^>]*>([\s\S]*?)<\/script>/)
                          ...
                          var saleValues = (((nd.inventoryView || {}).saleInformation || {})['$values']) || [];
                          var bidInfo    = ((nd.auctionInformation || {}).biddingInformation) || {};

                          console.log('[parse] attrs.Year:', attrs.Year, '| attrs.FuelTypeCode:', attrs.FuelTypeCode,
                                      '| attrs.DisplLiters:', attrs.DisplLiters, '| attrs.BranchState:', attrs.BranchState,
                                      '| attrs.BodyStyleName:', attrs.BodyStyleName, '| attrs.Segment:', attrs.Segment);
                          console.log('[parse] saleValues:', saleValues);
 
                          var acvVal = vm.parseDollars(vm.getVal(saleValues, 'ActualCashValue'));
                          var rcVal  = vm.parseDollars(vm.getVal(saleValues, 'EstimatedRepairCost'));
                          var bnp    = vm.parseDollars(vm.getVal(saleValues, 'BuyNowPrice')) || parseInt(attrs.BuyNowAmount || 0);
                          var minBid = parseInt(attrs.MinimumBidAmount || 0);

                          var filled = [];
                        
                          if (acvVal >= 500)  { vm.acv          = acvVal; filled.push('ACV $' + acvVal); }
                          if (rcVal  > 0)     { vm.repairCost   = rcVal;  filled.push('ремонт $' + rcVal); } else {
                            vm.repairCost = 0;
                          }
                          if (bnp    > 0)     { vm.buyNowPrice  = bnp;    filled.push('BuyNow $' + bnp); }
                          if (minBid > 0)     { vm.autoPricing.autoPrice = minBid; filled.push('мін.ставка $' + minBid); }


                          // ── Рік ────────────────────────────────────────────────────
                          var year = parseInt(attrs.Year || bidInfo.year);
                          if (year && manYearOptions.indexOf(year) !== -1) {
                            vm.customs.manufactureYear = year;
                            filled.push('рік ' + year);
                          }

                          // ── Тип пального ───────────────────────────────────────────
                          var fuelRaw = (attrs.FuelTypeCode || attrs.FuelTypeDesc || '').toLowerCase();
                          if (/gasoline|gas/.test(fuelRaw)) {
                            vm.customs.engineType = 'petrol';     filled.push('бензин');
                          } else if (/diesel/.test(fuelRaw)) {
                            vm.customs.engineType = 'diesel';     filled.push('дизель');
                          } else if (/electric|ev/.test(fuelRaw)) {
                            vm.customs.engineType = 'electric';   filled.push('електро');
                          }
                          ...
                          // ── Тип авто ───────────────────────────────────────────────
                          var bodyStyle = (attrs.BodyStyleName || attrs.Segment || '').toLowerCase();
                          if (/pickup|truck/.test(bodyStyle)) {
                            vm.autoShipping.vehicleType = 'pikap';  filled.push('пікап');
                          } else if (/sport utility|suv|crossover/.test(bodyStyle)) {
                            vm.autoShipping.vehicleType = 'suv';    filled.push('кросовер');
                          } else if (bodyStyle) {
                            vm.autoShipping.vehicleType = 'sedan';  filled.push('седан');
                          }

                          var priceFound = false;
                          if (!priceFound) {
                            var fallbackPrice = parseInt(attrs.MinimumBidAmount) || parseInt(bidInfo.buyNowAmount);
                          ...
                          if (stateCode && cityName) {
                            ...
                            locFound = autoLocation.filter(function (l) {
                              var n = l.name.toUpperCase();
                              return n.startsWith(stateCode + ' ') && n.indexOf(cityName) !== -1 && n.indexOf(auctionKey) !== -1;
                            })[0];
                          }
                          if (!locFound && stateCode) {
                            ...
                            locFound = autoLocation.filter(function (l) {
                              var n = l.name.toUpperCase();
                              return n.startsWith(stateCode + ' ') && n.indexOf(auctionKey) !== -1;
                            })[0];
                          }
                          if (locFound) {
                            vm.autoShipping.location.selected = locFound.id;
                            vm.locationSearch = locFound.name;
                            vm.$nextTick(function () { vm.onLocationChange(); });
                          ...
                          vm.customs.carrierInfo.color = (attrs.Color || attrs.PrimaryDamage || '').trim();
                          vm.customs.carrierInfo.transmission = (attrs.Transmission || '').trim();
                          var odo = parseInt((attrs.Odometer || attrs.OdometerKM || attrs.OdometerMiles || '').toString().replace(/[^0-9]/g, ''));
                          if (!isNaN(odo) && odo > 0) vm.customs.carrierInfo.mileage = odo;

                          // ── Make/Model для інформації ──────────────────────────────
                          var carLabel = [attrs.Year, attrs.Make, attrs.Model].filter(Boolean).join(' ');
                    ...
                    marketPriceDifference: function () {
                      return Math.round((this.customs.ukrainianMarketPrice || 0) - this.total());
                    },
                    ...
                    readMarketCache: function (cacheKey) {
                      try {
                        ...
                        if (!data || !data.ts || !data.medianPrice || (Date.now() - data.ts) > ttlMs) return null;
                        return data;
                      } catch (e) {
                        return null;
                      }
                    ...
                    buildAutoriaSearchUrl: function (target) {
                      var query = [target.make, target.model, target.year].filter(Boolean).join(' ');
                    ...
                    fetchViaProxy: async function (url) {
                      var proxy = (typeof CONFIG !== 'undefined' && CONFIG.proxyUrl) ? CONFIG.proxyUrl : '';
                      if (!proxy) throw new Error('Не задано CONFIG.proxyUrl для доступу до AUTO.RIA');
                      var ctrl = new AbortController();
                      var timeoutId = setTimeout(function () { ctrl.abort(); }, 15000);
                      try {
                        var resp = await fetch(proxy + encodeURIComponent(url), { signal: ctrl.signal });
                        if (!resp.ok) throw new Error('HTTP ' + resp.status);
                        var html = await resp.text();
                        if (!html || html.length < 2000) throw new Error('Коротка відповідь від проксі/сайту');
                        if (/just a moment|cloudflare|enable javascript/i.test(html.slice(0, 3000))) {
                    ...
                    extractListingsFromAutoriaHtml: function (html) {
                      ...
                      if (nextMatch && nextMatch[1]) {
                        try {
                          ...
                          while (stack.length) {
                            ...
                            if (p >= 500 && p <= 300000 && /auto_ria|auto\.ria|\/auto_/i.test(link)) {
                              if (link.indexOf('http') !== 0) link = 'https://auto.ria.com' + link;
                              addListing({
                                link: link,
                                title: title || '',
                                price: p,
                                year: parseInt(year || 0) || 0,
                                mileage: parseInt((mileage || '').toString().replace(/[^0-9]/g, '')) || 0,
                                fuel: (fuel || '').toString().toLowerCase(),
                                engineVolume: parseFloat((engineVolume || '').toString().replace(',', '.')) || 0,
                              });
                            }
                            for (var key in cur) {
                              if (Object.prototype.hasOwnProperty.call(cur, key)) stack.push(cur[key]);
                            }
                          }
                        } catch (e) {}
                      }
                      ...
                      if (!listings.length) {
                        var cardRe = /<a[^>]+href="([^"]*\/uk\/auto_[^"]+)"[^>]*>([\s\S]*?)<\/a>/gi;
                        var m;
                        while ((m = cardRe.exec(html)) !== null) {
                          var link = m[1].indexOf('http') === 0 ? m[1] : ('https://auto.ria.com' + m[1]);
                          var card = m[2] || '';
                          ...
                          var price = priceMatch ? parseInt(priceMatch[1].replace(/[^0-9]/g, '')) : 0;
                          addListing({
                            link: link,
                            title: titleMatch ? vm.decodeHtmlEntities(titleMatch[1]) : '',
                            price: price || 0,
                            year: yearMatch ? parseInt(yearMatch[1]) : 0,
                            mileage: mileMatch ? parseInt(mileMatch[1].replace(/[^0-9]/g, '')) : 0,
                            fuel: '',
                            engineVolume: 0,
                          });
                        }
                    ...
                    scoreSimilarity: function (listing, target) {
                      var score = 0;
                      var text = ((listing.title || '') + ' ' + (listing.link || '')).toLowerCase();
                      if (target.make && text.indexOf(target.make.toLowerCase()) !== -1) score += 35;
                      if (target.model && text.indexOf(target.model.toLowerCase()) !== -1) score += 45;
                      ...
                      if (target.engineType && listing.fuel) {
                        ...
                        if (target.engineType === 'electric' && /electric|електр/.test(lt)) score += 10;
                        if (target.engineType === 'petrol' && /gas|бенз|petrol/.test(lt)) score += 8;
                        if (target.engineType === 'diesel' && /diesel|дизел/.test(lt)) score += 8;
                      }
                    ...
                    lookupUkrainianPrice: async function () {
                      ...
                      try {
                        ...
                        if (!target.make || !target.model) {
                          ...
                          vm.marketMsg = '⚠ Недостатньо даних: потрібні make/model з лоту. Спочатку натисніть "Зчитати".';
                          return;
                        }
                        ...
                        if (cached) {
                          ...
                          vm.customs.marketCategory = cached.marketCategory || vm.getMarketCategoryByDiff(vm.customs.ukrainianMarketPrice - vm.total(), vm.total());
                          vm.marketStatus = 'ok';
                          vm.marketMsg = '✅ Використано кеш AUTO.RIA: ' + cached.sampleCount + ' оголошень, медіана $' + cached.medianPrice;
                          return;
                        }
                        ...
                        var filtered = withScore.filter(function (x) {
                          if (x.item.price < 500 || x.item.price > 300000) return false;
                          if (target.year && x.item.year && Math.abs(x.item.year - target.year) > 4) return false;
                          return x.score >= 25;
                        }).sort(function (a, b) {
                        ...
                        if (!filtered.length) {
                          vm.marketStatus = 'warn';
                          vm.marketMsg = '⚠ Знайдено оголошення, але немає релевантних для порівняння.';
                          return;
                        }

                        var selected = filtered.slice(0, 20);
                        var prices = selected.map(function (x) { return x.item.price; });
                        ...
                        vm.customs.marketCategory = vm.getMarketCategoryByDiff(diff, vm.total());
                        vm.marketStatus = selected.length < 3 ? 'warn' : 'ok';
                        vm.marketMsg = (selected.length < 3 ? '⚠ Мало даних: ' : '✅ ')
                          + 'знайдено ' + listings.length
                          + ', релевантних ' + filtered.length
                          + ', у медіані ' + selected.length
                          + ', медіана $' + median;
                        ...
                        vm.marketMsg = '❌ Помилка пошуку ціни на AUTO.RIA: ' + (err && err.message ? err.message : 'невідома помилка');
                      }
                    },
                    auctionFee: function () {
                      if (this.autoPricing.auctions.selected === auctions[0].id) {
                        if (this.autoPricing.autoPrice < 2000) {
                          return calculateCopartFee(this.autoPricing.autoPrice);
                        }
                        if (this.autoPricing.autoPrice >= 8000 && this.autoPricing.autoPrice < 10000) {
                          return calculateCopartFee(this.autoPricing.autoPrice) + 25;
                        }
                        if (this.autoPricing.autoPrice >= 15000) {
                          return calculateCopartFee(this.autoPricing.autoPrice) + 203;
                        }
                        return calculateCopartFee(this.autoPricing.autoPrice) + 15;
                      } else if (
                        this.autoPricing.auctions.selected === auctions[1].id
                      ) {
                        return calculateIaaIFee(this.autoPricing.autoPrice) + 15;
                      }
                    },
                    ...
                    commissionBank: function () {
                      ...
                      return (
                        Math.ceil(((this.autoPricing.autoPrice + this.auctionFee()) / 100 * 0.5)
                          + 30 + askss)
                      );
                    },
                    ...
                    strahovka: function () {
                      var strah = Math.ceil(((this.autoPricing.autoPrice + this.auctionFee()) / 100) * 2);
                      if (strah < 100) {
                        strah = 100;
                        return (strah);
                      }
                    ...
                    getCurrentPort: function () {
                      ...
                      return shippingPorts.filter(function (port) {
                        return port.id === portid;
                      })[0];
                    },
                    poshlina: function () {
                      return (
                        (this.autoPricing.autoPrice + this.auctionFee() + 1000) * 10
                      );
                    },

                    onLocationBlur: function () {
                      var vm = this;
                      setTimeout(function() { vm.locationDropOpen = false; }, 150);
                    },
                    onLocationChange: function () {
                      ...
                      var optimal = ports.reduce(function (acc, cur) {
                        return location.toPort[cur.id] < location.toPort[acc.id]
                          ? cur
                          : acc;
                      });
                    ...
                    mreo: function () {
                      ...
                      if (this.autoPricing.autoPrice < 13300) {
                        mreo_tmp = Math.ceil((this.autoPricing.autoPrice + this.auctionFee() + 1000) / 100 * 3 + 25);
                      } else if (this.autoPricing.autoPrice >= 13300 && this.autoPricing.autoPrice < 23500) {
                        mreo_tmp = Math.ceil((this.autoPricing.autoPrice + this.auctionFee() + 1000) / 100 * 4 + 25);
                      } else if (this.autoPricing.autoPrice >= 23500) {
                        mreo_tmp = Math.ceil((this.autoPricing.autoPrice + this.auctionFee() + 1000) / 100 * 5 + 25);
                      }
                      return mreo_tmp;
                    },
                    ...
                    totalShippingFee: function () {
                      var transportFee = this.getCurrentLocation().toPort[
                        this.autoShipping.shippingPort
                      ];

                      var location = this.getCurrentLocation();
                      var dostavka = location[this.autoPricing.auctions.selected] || 1100

                      var freightFee = this.getCurrentPort().toPort[
                        this.autoShipping.destinationPort.selected
                      ];
                      var vtFee =
                        this.autoShipping.vehicleType == vehicleType[1].id ? 0 : this.autoShipping.vehicleType == vehicleType[2].id ? 300 : 0;

                      // Океанський фрахт SUV → Одеса (2026)
                      var locName = (this.getCurrentLocation() ? this.getCurrentLocation().name : '').toUpperCase();
                      var isWest = /^(CA|WA|OR)\s/.test(locName);
                      var isGulf = /^(TX|FL|LA|MS|AL)\s/.test(locName);
                      var oceanKey = isWest ? 'west' : isGulf ? 'gulf' : 'east';
                      return dostavka + vtFee + this.oceanFreight[oceanKey];
                    },
                    ...
                    akcis: function () {
                      var age = currentYear - this.customs.manufactureYear;

                      if (
                        this.customs.engineType === engineType.Petrol ||
                        engineType.Petrol3
                      ) {
                        ...
                        this.customs.engineType === engineType.Diesel ||
                        engineType.Diesel3
                      ) {
                        ...
                        var res =
                          akcis +
                          this.totalAutoFee() * 0.2 +
                          this.totalAutoFee() * 0.1;
                        return Math.round(res);
                      }
                    },

                    totalCustomsFee: function () {
                      var engineVolumeNum = Number.parseFloat(
                        this.customs.engineVolume
                      );

                      var age = currentYear - this.customs.manufactureYear;


                      if (this.customs.engineType === engineType.Petrol) {
                        ...
                        // eur to usd
                        // akcis + nds + poshlina
                        var poshlina = (this.autoPricing.autoPrice + this.auctionFee() + 1000) / 100 * 10
                        var nds = (this.autoPricing.autoPrice + this.auctionFee() + 1000 + poshlina + akcis) / 100 * 20

                        var res = akcis + nds + poshlina// + 150
                        //  console.log("posl: " + poshlina + " | nds: " + nds + " | akcis: " + akcis);
                        return Math.round(res);
                      } else if (this.customs.engineType === engineType.Diesel) {
                        ...
                        var vat = (customsBase + duty + excisUsd) * 0.20;
                    ...
                    getVal: function (arr, keyName) {
                      var item = arr.find(function (x) { return x && x.key === keyName; });
...
function createMarketMethods(){
  var all=__createAllMethods();
  var pick=["parseAuctionLot","marketPriceDifference","getMarketCacheKey","getMarketCategoryByDiff","normalizeMarketTarget","readMarketCache","writeMarketCache","buildAutoriaSearchUrl","fetchViaProxy","extractListingsFromAutoriaHtml","decodeHtmlEntities","scoreSimilarity","computeMarketMedian","lookupUkrainianPrice"];
