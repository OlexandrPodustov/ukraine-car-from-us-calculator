  ...
  if (!sdkPromise) {
    sdkPromise = (async function () {
      ...
      try {
        ...
        // MODULE_NOT_FOUND тут читається як «скрипт зламався», хоча це просто
        // невстановлена залежність: пакет оголошений у package.json, але не
        // ставиться сам. Решта конвеєра (--photos, підписи, довідник цін)
        // працює без нього, тож кажемо це прямо.
        if (e && e.code === "MODULE_NOT_FOUND") {
          throw new Error(
            "@anthropic-ai/sdk не встановлено — виконайте `npm install`. " +
              "Фото лота качаються й без нього: scripts/damage-estimate.mjs --photos <id>",
          );
        }
        throw e;
      }
      if (!process.env.ANTHROPIC_API_KEY) {
        throw new Error(
          "ANTHROPIC_API_KEY не заданий — оцінка з фото не запуститься. " +
            "Кошторис можна ввести руками: PUT /api/lots/:id/ua-repair " +
